	...
	IN VOID *pRatesOrg)
{
	...
	for(IdLoop=0; IdLoop<NumOfChan; IdLoop++)
	{
		...
		if (IsRadarChannel(Cfg80211_Chan[IdLoop]))
		{
			...
			pChannels[IdLoop].flags |= (IEEE80211_CHAN_RADAR | 2);
		}
/*		CFG_TODO:
 ...
 */
	...
	if (pDriverBandInfo->RFICType & RFIC_24GHZ)
	{
		...
		CFG80211DBG(DBG_LVL_TRACE, ("80211> TxStream = %d\n", pDriverBandInfo->TxStream));
	...
	IN UINT32 BeaconLen)
{
	...
	pWiphy = pCfg80211_CB->pCfg80211_Wdev->wiphy;
	cfg80211_inform_bss_width(pWiphy,
				  pCfg80211_CB->pCfg80211_Channels,
				  NL80211_BSS_CHAN_WIDTH_20,
				  CFG80211_BSS_FTYPE_BEACON,
				  pBssid, 0, WLAN_CAPABILITY_IBSS,
				 beacon_interval, pBeacon, BeaconLen,
				 0, GFP_KERNEL);
/*
...
{
	cfg80211_ibss_joined(pNetDev, pBssid, NULL,GFP_KERNEL);
}
    ...
    IN VOID *pChDesc5G)
{
    ...
    CFG80211DBG(DBG_LVL_TRACE, ("80211> %s: TotalRuleNum=%d\n", __FUNCTION__, reg_rules_n));
     ...
     while (pChDesc && pChDesc->FirstChannel)
    {
        ...
        /*
        CFG80211DBG(DBG_LVL_TRACE, ("rule_idx=%d, srart_freq=%u, end_freq=%d, bw=%d flags=%d\n",
